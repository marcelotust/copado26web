...
export type ScanLogEntry = { code: string; id: string | null; ts: number }

function parseCodeToStickerId(rawCode: string): string | null {
...
export function useScannerLog() {
  ...
  const addEntry = useCallback(async (code: string): Promise<string | null> => {
  ...
  const handleManualSubmit = useCallback(async (e: FormEvent) => {
    e.preventDefault()
