...
function SwapCard({ sticker }) {
  ...
  return (
    <div className={`relative flex items-center gap-3 bg-gradient-to-r ${gradientClasses(sticker.teamCode)} rounded-xl px-3 py-2.5 shadow`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-white/70 text-xs truncate">{section?.name ?? sticker.teamCode}</p>
      </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 pt-4 pb-3 border-b border-slate-800 shrink-0">
        ...
        <div>
          <h2 className="text-white font-bold text-lg">Swaps</h2>
          <p className="text-slate-400 text-xs">
            {swaps.length} duplicate sticker{swaps.length !== 1 ? 's' : ''} to trade
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {swaps.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-center gap-3">
            ...
            <p className="text-slate-400 font-semibold">No duplicates yet!</p>
            <p className="text-slate-600 text-sm">Keep collecting stickers and your extras will appear here.</p>
          </div>
        ) : (
              ...
              return (
                <div key={teamCode}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={`text-xs font-bold tracking-wider text-${teamColor(teamCode)}-300`}>
                      {section?.name ?? teamCode}
                    </span>
                    <span className="text-slate-600 text-xs">· {byTeam[teamCode].length} dupes</span>
                  </div>
