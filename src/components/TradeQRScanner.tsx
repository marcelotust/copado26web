...
export default function TradeQRScanner({ onScanned }: Props) {
  ...
  if (denied) {
    return (
      <div className='flex flex-col gap-3 py-2'>
        ...
        />
        {invalid && <p className='text-rose-400 text-xs' role='alert'>{t('trade.scanInvalid')}</p>}
        <button
          type='button'
  ...
  return (
    <Suspense fallback={<div className='text-slate-400 text-sm text-center py-8'>{t('trade.scanLoading')}</div>}>
      ...
      <p className='text-slate-500 text-xs text-center mt-2'>{t('trade.scanHint')}</p>
      {invalid && <p className='text-rose-400 text-xs text-center mt-1' role='alert'>{t('trade.scanInvalid')}</p>}
    </Suspense>
  )
}
