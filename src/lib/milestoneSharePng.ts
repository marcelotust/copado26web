...
export async function shareOrDownloadPng(blob: Blob, title: string, text?: string): Promise<void> {
  const file = new File([blob], 'meu-album-2026-conquista.png', { type: 'image/png' })
  const shareData: ShareData = text ? { files: [file], title, text } : { files: [file], title }
